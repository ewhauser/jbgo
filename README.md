...
`make build`, `make test`, and `make lint` cover the Go modules. `make test` now resolves the pinned GNU `diff` oracle through Nix via `./scripts/ensure-diffutils.sh`; set `GBASH_CONFORMANCE_DIFF` if you want to override that path explicitly. `make conformance-test` resolves the pinned bash and curl oracles through Nix via `./scripts/ensure-bash.sh` and `./scripts/ensure-curl.sh`; set `GBASH_CONFORMANCE_BASH` or `GBASH_CONFORMANCE_CURL` to override those paths explicitly. `make contrib-conformance-test` runs the contrib parity suites, currently the package-local jq and awk corpora under `contrib/jq` and `contrib/awk`, using pinned bash, jq, and awk oracles from `./scripts/ensure-bash.sh`, `./scripts/ensure-jq.sh`, and `./scripts/ensure-awk.sh`; set `GBASH_CONFORMANCE_BASH`, `GBASH_CONFORMANCE_JQ`, or `GBASH_CONFORMANCE_AWK` to override those paths explicitly. See [CONTRIBUTING.md](./CONTRIBUTING.md) for repository structure, module versioning, release process, benchmarks, and GNU coreutils compatibility testing.
