...
- `dircolors` follows the uutils/GNU surface for `-b/--sh`, `-c/--csh`, `-p/--print-database`, `--print-ls-colors`, optional config-file or stdin input, shell autodetection via `SHELL`, TERM/COLORTERM-gated config sections, and shared default LS_COLORS tables intended for later `ls --color` parity work
