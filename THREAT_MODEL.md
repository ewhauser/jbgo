...
- Runtime and session orchestration: `internal/runtime/runtime.go:New` creates the default registry, engine, policy, network client, base environment, and session factory; `internal/runtime/session.go:Session.exec` resolves cwd and env, applies output caps, and delegates to the shell engine.
- Shell execution adapter: `internal/shell/mvdan.go:MVdan.Run` constructs a fresh `interp.Runner` with explicit env, stdio, open/stat/readdir handlers, call handling, and command execution handlers; unknown commands return `127` in `internal/shell/mvdan.go:execHandler`.
...
- Untrusted script or local caller -> CLI or embedding API -> `Runtime.Run` / `Session.Exec`
  ...
  - Validation and enforcement: shell parsing and runner setup in `internal/shell/mvdan.go`, default policy and budgets in `internal/runtime/runtime.go:New`, cwd and env normalization in `internal/runtime/session.go:Session.exec`.
...
- Runtime session -> policy and registry -> shell builtins and commands
  ...
  - Channel/protocol: in-process handler callbacks inside `mvdan/sh`.
  ...
  - Validation and enforcement: `internal/shell/mvdan.go:execHandler`, `policy/policy.go`, and `commands/invocation_capabilities.go`.
...
|---|---|---|
| `internal/shell/mvdan.go` | Central command resolution, builtin mediation, nested execution, and handler wiring for the forked shell engine. | TM-004, TM-006, TM-007 |
