...
- Runtime/session layer: `internal/runtime/runtime.go` / `Runtime.New`, `Runtime.NewSession`, and `internal/runtime/session.go` / `Session.exec` build the session filesystem, registry, policy, environment, output buffers, and trace recorder.
...
- Optional network client: `network/network.go` / `HTTPClient.Do` implements allowlisted HTTP egress and is exposed only when the runtime is configured with a network client, which also registers `curl` in `internal/runtime/runtime.go` / `New` and `commands/curl.go` / `Run`.
- Trace output: `trace/trace.go` defines event payloads returned in `ExecutionResult.Events`; `internal/runtime/session.go` wires a trace buffer into each execution.
...
- Cannot use network egress unless the embedder injects a network client; `curl` fails closed if `inv.Net` is nil. Evidence anchors: `internal/runtime/runtime.go` / `New`, `commands/curl.go` / `Run`.
...
| Trace/event return path | `ExecutionResult.Events` returned to caller | Runtime -> orchestrator/log sink | Sensitive if argv or file paths contain credentials or secrets. | `internal/runtime/session.go` / `Session.exec`; `trace/trace.go` / `CommandEvent` |
...
| `internal/runtime/session.go` | Binds the live session filesystem, policy, network client, and trace buffer into each execution. | TM-001, TM-002, TM-004, TM-005 |
