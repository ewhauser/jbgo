...
make bench-fs BENCH_FS_PROFILE=synthetic  # fast local synthetic profile
make bench-fs BENCH_FS_KERNEL_CACHE_DIR=stream  # stream kernel tarball when local disk is tight
make bench-fs-artifacts BENCH_FS_KERNEL_CACHE_DIR=stream  # prebuild reusable Bleve and SQLite artifacts
```
...
`make bench-fs` runs the filesystem benchmark harness. By default it uses the pinned `linux-kernel` profile, downloads and verifies `linux-6.19.8.tar.xz` from kernel.org as needed, reuses cached Bleve and SQLite benchmark artifacts when available, compares the `memory`, `overlay`, `sqlite`, and `bleve` filesystem backends on both cold and warm search scenarios, and writes JSON to `website/content/performance/filesystem-benchmark-data.json`.
