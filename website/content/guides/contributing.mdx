...
| `shell` | Shell AST parsing and execution via `mvdan/sh` |
...
```bash
npm exec --yes pnpm@10.10.0 -- install --frozen-lockfile
```
...
Published versions are coordinated. The root module uses plain tags (`v0.0.10`); contrib modules use nested-module tags (`contrib/jq/v0.0.10`). Child modules maintain real version requirements in `go.mod` plus local `replace` directives for development.
