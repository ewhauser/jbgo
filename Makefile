...
test:
	...
	RIPGREP_PATH=$$(./scripts/ensure-ripgrep.sh) || exit 1; \
	GBASH_CONFORMANCE_DIFF="$$DIFF_PATH" \
	GBASH_CONFORMANCE_RIPGREP="$$RIPGREP_PATH" \
...
contrib-conformance-test:
	...
	AWK_PATH=$$(./scripts/ensure-awk.sh) || exit 1; \
	YQ_PATH=$$(./scripts/ensure-yq.sh) || exit 1; \
	GBASH_RUN_JQ_CONFORMANCE=1 GBASH_CONFORMANCE_BASH="$$BASH_PATH" \
	GBASH_CONFORMANCE_JQ="$$JQ_PATH" \
	  go test ./contrib/jq -run '^TestJQConformance$$' -count=1 -timeout=20m || exit 1; \
	GBASH_RUN_AWK_CONFORMANCE=1 GBASH_CONFORMANCE_BASH="$$BASH_PATH" \
	GBASH_CONFORMANCE_AWK="$$AWK_PATH" \
	  go test ./contrib/awk -run '^TestAWKConformance$$' -count=1 -timeout=20m || exit 1; \
	GBASH_RUN_YQ_CONFORMANCE=1 GBASH_CONFORMANCE_BASH="$$BASH_PATH" \
	GBASH_CONFORMANCE_YQ="$$YQ_PATH" \
	  go test ./contrib/yq -run '^TestYQConformance$$' -count=1 -timeout=20m

ensure-awk:
	@./scripts/ensure-awk.sh
