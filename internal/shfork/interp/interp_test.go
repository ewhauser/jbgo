...
import (
	...
	"github.com/ewhauser/gbash/internal/shfork/internal"
...
set +o pipefail
 #IGNORE`,
	...
	*) echo "invalid option $REPLY"; break;;
	...
	{"mkdir -p _tmp; >_tmp/ex; chmod +x _tmp/ex; type _tmp/ex", "_tmp/ex is _tmp/ex\n"},
	{"mkdir -p _tmp; >_tmp/date; chmod +x _tmp/date; PATH=_tmp:$PATH; type date", "date is _tmp/date\n"},
	...
	{"interp_foo(){ echo hi; }; type interp_foo | grep -q 'echo hi'", ""},
	{"mkdir bin; >bin/interp_foo; chmod +x bin/interp_foo; PATH=$PWD/bin; interp_foo(){ :; }; out=$(type -a interp_foo); case $out in *'interp_foo is a function'*\"$PWD/bin/interp_foo\"*) echo ok;; *) echo bad;; esac", "ok\n"},
	{"mkdir bin; >bin/interp_foo; chmod +x bin/interp_foo; PATH=$PWD/bin; interp_foo(){ :; }; out=$(type -af interp_foo); case $out in *\"$PWD/bin/interp_foo\"*) echo ok;; *) echo bad;; esac", "ok\n"},
	...
	{"f=$PWD/f; x=$(echo side >\"$f\") export y=1; cat \"$f\"; printf 'x=<%s> y=<%s>\\n' \"$x\" \"$y\"", "side\nx=<> y=<1>\n"},
