...
func mustFuzzCommandMetadata(tb testing.TB) []fuzzCommandMetadata {
	...
	specs := []fuzzCommandMetadata{
		...
		fuzzSpec("compadjust", "shell",
			fuzzVariant("", "completion:adjust", "words"),
			fuzzVariant("", "completion:adjust", "cur", "prev", "words", "cword"),
		),
		fuzzSpec("compgen", "shell",
			fuzzVariant("", "completion:action", "-A", "builtin", "ec"),
			fuzzVariant("", "completion:wordlist", "-W", "one two", "o"),
		),
