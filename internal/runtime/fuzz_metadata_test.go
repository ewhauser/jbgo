...
func mustFuzzCommandMetadata(tb testing.TB) []fuzzCommandMetadata {
	...
	specs := []fuzzCommandMetadata{
		...
		fuzzSpec("complete", "shell",
			fuzzVariant("", ""),
			fuzzVariant("", "wordlist", "-W", "alpha beta", "cmd"),
			fuzzVariant("", "function", "-F", "handler", "cmd"),
			fuzzVariant("", "command", "-C", "generator", "cmd"),
			fuzzVariant("", "action", "-A", "builtin", "cmd"),
			fuzzVariant("", "option", "-o", "nospace", "cmd"),
			fuzzVariant("", "flag:p", "-p"),
			fuzzVariant("", "flag:r", "-r", "cmd"),
			fuzzVariant("", "flag:D", "-D", "-F", "handler"),
		),
		fuzzSpec("touch", "file",
