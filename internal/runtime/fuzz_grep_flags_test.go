...
func FuzzGrepFlagsCommand(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, rawData []byte) {
		...
		writeSessionFile(t, session, "/tmp/grep-bre.txt", []byte("x+y\nxy\n"))
		...
		script := []byte(
			"grep 'x\\+y' /tmp/grep-bre.txt >/tmp/grep-bre.out || true\n" +
				"grep -E 'x+y' /tmp/grep-bre.txt >/tmp/grep-ere.out || true\n" +
				...
				"grep -f /tmp/grep-pattern-file.txt /tmp/grep-pattern-input.txt >/tmp/grep-pattern-file.out || true\n" +
