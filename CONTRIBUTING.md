...
Use `make fix-modules MODULE_VERSION=vX.Y.Z` when preparing the next coordinated root plus contrib release line. That updates the nested module requirements, refreshes the local replaces, and runs `go mod tidy` in each child module.
