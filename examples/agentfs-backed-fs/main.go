...
import (
	...
	"github.com/ewhauser/gbash/examples/internal/agentfsfs"
)
...
func run(ctx context.Context, stdin io.Reader, stdout, stderr io.Writer, args []string) (int, error) {
	...
	gb, err := agentfsfs.NewRuntime(ctx, opts.dbPath, opts.workDir)
	if err != nil {
	...
	result, err := gb.Run(ctx, execRequest(script))
	if err != nil {
...
func parseCLIOptions(stderr io.Writer, args []string) (cliOptions, error) {
	...
	fs := flag.NewFlagSet("agentfs-backed-fs", flag.ContinueOnError)
	fs.SetOutput(stderr)
	fs.StringVar(&opts.dbPath, "db", "", "AgentFS SQLite database file used to persist the sandbox filesystem")
