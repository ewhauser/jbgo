...
This example implements a custom `jbfs.FileSystem` on top of a host SQLite database file and wires it into `jbruntime.CustomFileSystem(...)`.
...
```bash
go run ./examples/sqlite-backed-fs --db /tmp/jbgo-sandbox.db --script "printf 'hello from sqlite fs\n' > /tmp/hello.txt"
```
...
```bash
cd examples
make run-sqlite-backed-fs SQLITE_FS_DB=/tmp/jbgo-sandbox.db SQLITE_FS_SCRIPT="printf 'hello from sqlite fs\n' > /tmp/hello.txt"
```
...
```bash
cd examples
make run-sqlite-backed-fs-repl SQLITE_FS_DB=/tmp/jbgo-sandbox.db
```
...
```bash
cat <<'EOF' | go run ./examples/sqlite-backed-fs --db /tmp/jbgo-sandbox.db
pwd
ls /tmp
cat /tmp/hello.txt
EOF
...
```bash
go run ./examples/sqlite-backed-fs --db /tmp/jbgo-sandbox.db --workdir /home/agent/project
```
